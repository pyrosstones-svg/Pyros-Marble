...
export default function PorcelainPage() {
  ...
  const porcelainStones = stones.filter(stone => 
    (stone.category === 'porcelain' || /porcelain/i.test(stone.name)) &&
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-10">
          <div className="space-y-4 max-w-2xl text-left">
            ...
            <p className="font-inter text-sm sm:text-base text-neutral-300 font-light leading-relaxed">
              Ultra-durable, lightweight 6mm, 9mm & 12mm Porcelain Slabs (1200×2800 mm & 1600×3200 mm). Zero porosity, stain-proof, UV resistant surfaces for modern kitchens, facades, and luxury flooring.
            </p>
            <div className="flex flex-wrap gap-4 pt-2">
              <span className="text-xs font-mono text-[#D4AF37] flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4 text-[#D4AF37]" /> 0.01% Zero Porosity
              </span>
              <span className="text-xs font-mono text-neutral-300 flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4 text-[#D4AF37]" /> Jumbo Sizes 1600×3200 mm
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-10 pb-6 border-b border-[#E5E7EB]">
          <div>
            <span className="font-mono text-xs text-[#D4AF37] font-bold uppercase tracking-widest">
              Dedicated Porcelain Page
            </span>
            <h2 className="font-cormorant text-3xl font-medium text-[#1C1C21]">
              Browse Porcelain Slabs Collection ({porcelainStones.length} Varieties)
            </h2>
          </div>
          ...
          <div className="relative w-full sm:w-80">
            ...
            <input 
              type="text" 
              placeholder="Search Porcelain by name..."
              value={searchQuery}
