...
export default function SlatePage() {
  ...
  const slateStones = stones.filter(stone => 
    (stone.category === 'slate' || /slate/i.test(stone.name)) &&
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-10">
          <div className="space-y-4 max-w-2xl text-left">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#D4AF37]/15 border border-[#D4AF37]/40 rounded-md text-[#D4AF37] text-xs font-mono font-bold tracking-widest uppercase">
              🧱 Architectural Textured Slate
            </div>
            <h1 className="font-cormorant text-4xl sm:text-6xl font-semibold leading-tight">
              Indian Slate Stone Collection
            </h1>
            <p className="font-inter text-sm sm:text-base text-neutral-300 font-light leading-relaxed">
              Deoli Green, Jack Black, Indian Copper & Silver Shine Slate tiles and flexi-veneers. Ideal for modern feature walls, exterior facade cladding, and interior flooring accents.
            </p>
            <div className="flex flex-wrap gap-4 pt-2">
              <span className="text-xs font-mono text-[#D4AF37] flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4 text-[#D4AF37]" /> Natural Cleft Texture
              </span>
              <span className="text-xs font-mono text-neutral-300 flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4 text-[#D4AF37]" /> Calibrated & Thin Veneer Available
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-10 pb-6 border-b border-[#E5E7EB]">
          <div>
            <span className="font-mono text-xs text-[#D4AF37] font-bold uppercase tracking-widest">
              Dedicated Slate Page
            </span>
            <h2 className="font-cormorant text-3xl font-medium text-[#1C1C21]">
              Browse Slate Stone Collection ({slateStones.length} Varieties)
            </h2>
          </div>
          ...
          <div className="relative w-full sm:w-80">
            ...
            <input 
              type="text" 
              placeholder="Search Slate by name..."
              value={searchQuery}
